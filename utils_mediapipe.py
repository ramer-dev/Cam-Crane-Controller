...
class MediaPipeBody:
    def __init__(self, static_image_mode=True, model_complexity=1, intrin=None):
        ...
        self.param = {
            ...
            'visible': np.zeros(33),
            # Visibility: Likelihood [0,1] of being visible (present and not occluded) in the image
            'fps': -1,  # Frame per sec
        }
...
class MediaPipeHolistic:
    ...
    def convert_relative_to_actual_3d_joint(self, param_fc, param_lh, param_rh, param_bd, intrin):
        ...
        if param_fc['detect']:
            ...
            d = np.linalg.norm(param_fc['joint_3d'][386] - param_fc['joint_3d'][
                159])  # Dist btw left and right eye (in relative 3D coor)
        ...
        if param_lh['class'] is not None:
            ...
            D = 0.08  # Note: Hardcode actual dist btw wrist and index finger MCP as 0.08 m
            d = np.linalg.norm(
                param_lh['joint_3d'][0] - param_lh['joint_3d'][9])  # Dist btw wrist and index finger MCP joint
        ...
        if param_rh['class'] is not None:
            ...
            D = 0.08  # Note: Hardcode actual dist btw wrist and index finger MCP as 0.08 m
            d = np.linalg.norm(
                param_rh['joint_3d'][0] - param_rh['joint_3d'][9])  # Dist btw wrist and index finger MCP joint
...
class MediaPipeObjectron:
    def __init__(self, static_image_mode=True, max_num_objects=5, model_name='Shoe', intrin=None):
        ...
        for i in range(max_num_objects):
            p = {
                'detect': False,  # Boolean to indicate whether a face is detected
                'landmarks_2d': np.zeros((9, 2)),
                # 2D landmarks of object's 3D bounding box in image coordinate (pixel) Note: first value is center of bounding box, remaining 8 values are corners of bounding box
                'landmarks_3d': np.zeros((9, 3)),
                # 3D landmarks of object's 3D bounding box in camera coordinate Note: first value is center of bounding box, remaining 8 values are corners of bounding box
                'rotation': np.eye(3),  # Rotation matrix from object coordinate frame to camera coordinate frame
                'translation': np.zeros(3),
                # Translation vector from object coordinate frame to camera coordinate frame
